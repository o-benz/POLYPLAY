...
export default function CreatePlaylist() {
  ...
  const handleNameChange = (event) => {setData({...data, name : event.target.value})};

  // TODO : Gérer le changement de description
  const handleDescriptionChange = (event) => {setData({...data, description : event.target.value})};
      ...
      <form className="form-group" id="playlist-form">
        ...
        <fieldset className="form-control">
          ...
          <datalist id="song-dataList">
            {
            songs.map((song) => { 
            return <option key = {song.id} value={song.name} />
            })}
          </datalist>
